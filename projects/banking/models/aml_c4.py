...
def define_aml(model: SystemLandscape) -> SoftwareSystem:
    ...
    _ = comp + Container(
        "Sanctions Screening", "Screens parties against sanctions lists", technology="Python"
    )
...
def link_aml(model: SystemLandscape) -> None:
    ...
    model.relate(
        comp["Sanctions Screening"], comp["Case Manager"], "Creates sanctions investigation case"
    )
